 ...
 *
 * build-env.test.ts proves the precedence rules on the pure function. This proves the two
 * things that function cannot: that every Next-running script goes through the wrapper, and
 * that the wrapper, run as a process, applies those rules and reaches the real Next binary.
 */
...
describe('package scripts run Next through the wrapper', () => {
  const scripts = (
    JSON.parse(readFileSync(path.join(WEB_DIR, 'package.json'), 'utf8')) as {
  ...
  it('never invokes next directly', () => {
    ...
    expect(scripts['dev']).toBe('node ./scripts/with-env.mjs dev --port 3100');
    expect(scripts['start']).toBe('node ./scripts/with-env.mjs start --port 3100');
    expect(scripts['build']).toBe('node ./scripts/with-env.mjs build');
    expect(scripts['typecheck']).toMatch(/^node \.\/scripts\/with-env\.mjs typegen/);
  });
});
...
describe('the wrapper as a process', () => {
  ...
  function run(env: Record<string, string>): {
    ...
    const result = spawnSync(process.execPath, [WRAPPER, '--version'], {
      cwd: WEB_DIR,
      env: { ...env, CAPITALDESK_WEB_ENV_DIR: envDir },
      encoding: 'utf8',
      timeout: 60_000,
    });
  ...
  it('lets the explicit process environment beat the env file, at the process level', () => {
    ...
    expect(ran.status, ran.stderr).toBe(0);
    const applied = ran.stderr.match(/local defaults applied for: (.*)/)?.[1] ?? '';
    // Neither the origin (explicit) nor the build id (mapped from CAPITALDESK_BUILD_ID) is defaulted.
    expect(applied).not.toContain('NEXT_PUBLIC_CAPITALDESK_API_BASE_URL');
