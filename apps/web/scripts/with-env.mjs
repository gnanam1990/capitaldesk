 ...
 *
 * Because `start` runs in production, everything this file imports is a production
 * dependency of the package, and a test checks that too: a wrapper that only works after a
 * development install would fail exactly where it matters.
 *
...
// Env files are read from the working directory - the same place Next itself reads them, and
// the package directory whenever a package script runs. There is no override: a test that
// needs known files runs the wrapper with that directory as its cwd.
