...
export interface OpeningBalance {
  ...
  /** Total units the account holds: free plus locked, as one number. */
...
export class BaselineRepository {
  ...
  bootstrap(input: BootstrapInput): Promise<BootstrapOutcome> {
    return serializable(this.pool, async (client): Promise<BootstrapOutcome> => {
      ...
      // A replay of the same request is the same answer, not a second opening.
      const existing = await client.query<{ cut_id: string; ledger_txn_id: string | null }>(
        `SELECT cut_id, ledger_txn_id FROM account_baselines
          WHERE workspace_id = $1 AND pool_id = $2 AND baseline_id = $3`,
        [input.workspaceId, input.poolId, input.baselineId],
      );
      const stored = existing.rows[0];
      if (stored !== undefined) {
        return stored.cut_id === input.cutId
          ? {
              ok: true,
              replayed: true,
      ...
      const preconditions = await readPreconditions(client, input, poolRow);
      if (preconditions === null) return { ok: false, reason: 'UNKNOWN_CUT' };
      ...
      const ledgerTxnId = `baseline-${input.baselineId}`;
      const entries = input.balances
        .filter((balance) => balance.atoms !== 0n)
        .flatMap((balance) => [
          {
  ...
  allocate(input: AllocationInput): Promise<AllocationOutcome> {
    return serializable(this.pool, async (client): Promise<AllocationOutcome> => {
      ...
      const existing = await client.query<{
        ...
        `SELECT revision, ledger_txn_id, from_owner, to_owner, atoms::text, asset_code, asset_scale
      ...
      const ledgerTxnId = `allocation-${input.allocationId}`;
      const posted = await LedgerRepository.postOn(client, {
        workspaceId: input.workspaceId,
        poolId: input.poolId,
        epoch: input.epoch,
...
/** Everything the readiness predicate needs, read under the caller's lock. */
async function readPreconditions(
  client: Queryable,
  input: BootstrapInput,
  poolRow: { stable_account_id: string; environment: string },
): Promise<BaselinePreconditions | null> {
  const cut = await client.query<{
    coverage_state: string;
    detection_scope: string;
    stable_account_id: string;
  }>(
    `SELECT c.coverage_state, c.detection_scope, s.stable_account_id
  ...
  const cutRow = cut.rows[0];
  if (cutRow === undefined) return null;
  ...
  return {
    coverageState: cutRow.coverage_state as BaselinePreconditions['coverageState'],
    detectionScope: cutRow.detection_scope as BaselinePreconditions['detectionScope'],
    cutStableAccountId: cutRow.stable_account_id,
    ...
    observedAssets: input.balances.map((balance) => balance.asset),
