...
describeIfDatabase('nothing in module 04 can reach NOT_SENT_PROVEN', () => {
  ...
  it('refuses an INSERT that starts an attempt already in the state', async () => {
    // The transition trigger fires on UPDATE. An attempt written straight into the state
    // would bypass it, so the marked-state CHECK has to refuse the row itself.
    let refusal = 'accepted';
    try {
      await harness.admin.query(
        `INSERT INTO dispatch_attempts
           (workspace_id, pool_id, epoch, plan_id, attempt_id, client_order_id, dispatch_token, state)
         VALUES ($1, $2, 1, 'plan-1', 'a-forged', 'client-forged', 'token-forged', 'NOT_SENT_PROVEN')`,
        [WORKSPACE, POOL],
      );
    } catch (error) {
      refusal = sqlState(error);
