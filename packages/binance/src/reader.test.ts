...
/**
 * A reader that has already proven its account.
 *
 * Every authenticated read but the snapshot itself now requires that proof, so a fixture
 * exercising orders or trades has to establish it the same way production does.
 */
async function provenVenue(script: Record<string, (url: URL) => Response>): Promise<{
  reader: BinanceSpotReader;
  calls: URL[];
}> {
  const built = venue({ '/api/v3/account': () => json(ACCOUNT_BODY), ...script });
  await built.reader.accountSnapshot(ASSET_SCALES);
...
describe('account identity binding', () => {
  ...
  it('refuses to construct when the environment and the deployment disagree', () => {
    // Both claims end up in evidence: the reader stamps its environment onto every
    // observation while the transport decides which host is contacted. Nothing connected
    // them, so a testnet reader could stamp production onto testnet facts.
    const transport = new ReadOnlyTransport({
      deployment: 'testnet',
      origin: TESTNET,
    ...
    for (const environment of ['production', 'local'] as const) {
      expect(
        () =>
          new BinanceSpotReader({
            transport,
            deployment: 'testnet',
            identity: { environment, expectedStableAccountId: '354937868', epoch: 3 },
          }),
        environment,
      ).toThrow(ContractViolation);
    }
    // The positive control: the pair that agrees constructs.
    expect(
      () =>
        new BinanceSpotReader({
          transport,
   ...
   * configuration means a credential pointing elsewhere produces facts filed under an account
   * nobody confirmed.
   */
  describe('the account must be proven before it can be stamped', () => {
    it('refuses an order, open-order scan or trade read before any snapshot', async () => {
      const { reader } = venue({
        '/api/v3/order': () => json({}),
        '/api/v3/openOrders': () => json([]),
        '/api/v3/myTrades': () => json([]),
      });
      for (const attempt of [
        () => reader.orderByIdentity({ symbol: 'BTCUSDT', venueOrderId: '1' }, SCALES),
        () => reader.openOrdersAccountWide(),
        () => reader.tradesFrom('BTCUSDT', SCALES),
      ]) {
        const failure = await attempt().catch((error: unknown) => error);
        expect((failure as ContractViolation).reason).toBe('IDENTITY_UNSTABLE_ACCOUNT');
      }
      expect(reader.provenAccountId).toBeNull();
    });

    it('stamps the proven account, not the configured one, once a snapshot succeeds', async () => {
      const built = await provenVenue({ '/api/v3/openOrders': () => json([]) });
      expect(built.reader.provenAccountId).toBe('354937868');
      const observed = await built.reader.openOrdersAccountWide();
      expect(observed.provenance.stableAccountId).toBe('354937868');
    });

    it('does not become proven when the snapshot was refused', async () => {
      const { reader } = venue({
        '/api/v3/account': () => json({ ...ACCOUNT_BODY, uid: 999_999_999 }),
        '/api/v3/openOrders': () => json([]),
      });
      await expect(reader.accountSnapshot(ASSET_SCALES)).rejects.toBeInstanceOf(ContractViolation);
      expect(reader.provenAccountId).toBeNull();
      await expect(reader.openOrdersAccountWide()).rejects.toBeInstanceOf(ContractViolation);
    });

    it('still allows the public market read, which stamps no account of its own', async () => {
      const { reader } = venue({
        '/api/v3/exchangeInfo': () =>
          json({
            symbols: [
      ...
      await expect(reader.marketContext('BTCUSDT')).resolves.toMatchObject({
...
describe('order lookup by exact identity', () => {
  ...
  it('looks an order up by the venue id, and by our client id', async () => {
    const { reader, calls } = await provenVenue({
      '/api/v3/order': () => json(ORDER),
    });
  ...
  it('reports a NOT_FOUND as a typed rejection with the venue code', async () => {
    const { reader } = await provenVenue({
  ...
  it('refuses an answer about a different order id than the one asked about', async () => {
    // The symbol matching is not enough: this result is the sole evidence for one specific
    // dispatch, and an answer about another order would be recorded as that dispatch's
    // outcome.
    const { reader } = await provenVenue({
      '/api/v3/order': () => json({ ...ORDER, orderId: 999 }),
    });
    const failure = await reader
      .orderByIdentity({ symbol: 'BTCUSDT', venueOrderId: '100234' }, SCALES)
      .catch((error: unknown) => error);
    expect((failure as ContractViolation).reason).toBe('IDENTITY_SCOPE_MISMATCH');
    expect((failure as ContractViolation).detail['returned']).toBe('999');
  });

  it('refuses an answer about a different client order id than the one asked about', async () => {
    const { reader } = await provenVenue({
      '/api/v3/order': () => json({ ...ORDER, clientOrderId: 'someone-elses' }),
    });
    const failure = await reader
      .orderByIdentity({ symbol: 'BTCUSDT', clientOrderId: 'capitaldesk-1' }, SCALES)
      .catch((error: unknown) => error);
    expect((failure as ContractViolation).reason).toBe('IDENTITY_SCOPE_MISMATCH');
  });

  it('refuses an answer carrying no client order id when one was asked for', async () => {
    const { clientOrderId: _drop, ...noClient } = ORDER;
    const { reader } = await provenVenue({
      '/api/v3/order': () => json(noClient),
    });
    await expect(
      reader.orderByIdentity({ symbol: 'BTCUSDT', clientOrderId: 'capitaldesk-1' }, SCALES),
    ).rejects.toBeInstanceOf(ContractViolation);
  });

  it('accepts the exact order it asked for, by either identity', async () => {
    ...
    await expect(
      reader.orderByIdentity({ symbol: 'BTCUSDT', venueOrderId: '100234' }, SCALES),
    ).resolves.toMatchObject({ value: { venueOrderId: '100234' } });
    await expect(
      reader.orderByIdentity({ symbol: 'BTCUSDT', clientOrderId: 'capitaldesk-1' }, SCALES),
    ).resolves.toMatchObject({ value: { clientOrderId: 'capitaldesk-1' } });
  });

  it('refuses an answer about a different symbol than the one asked about', async () => {
    const { reader } = await provenVenue({
      '/api/v3/order': () => json({ ...ORDER, symbol: 'ETHUSDT' }),
    });
    const failure = await reader
      .orderByIdentity({ symbol: 'BTCUSDT', venueOrderId: '100234' }, SCALES)
      .catch((error: unknown) => error);
  ...
  it('preserves an unknown status as quarantined rather than mapping it', async () => {
    ...
    const { reader: other } = await provenVenue({
...
describe('trade pagination', () => {
  ...
  it('treats a short page as the end of the range', async () => {
    const { reader } = await provenVenue({
      '/api/v3/myTrades': () => json([trade(10), trade(11)]),
    });
    const page = await reader.tradesFrom('BTCUSDT', SCALES, { limit: 3 });
  ...
  it('compares ids numerically, so 9 does not sort after 10', async () => {
    const { reader } = await provenVenue({
      '/api/v3/myTrades': () => json([trade(9), trade(10)]),
    });
  ...
  it('never infers a gap from the numeric distance between two ids', async () => {
    ...
    const { reader } = await provenVenue({
      '/api/v3/myTrades': () => json([trade(10), trade(900)]),
    });
  ...
  it('refuses a zero page size before it is sent', async () => {
    // The previous version of this case claimed to prove that a full page with no usable id
    // is refused, but `limit: 0` never reaches that branch — the transport refuses the
    // parameter first. It asserted the wrong mechanism, so it is stated as what it is.
    const { reader } = await provenVenue({ '/api/v3/myTrades': () => json([]) });
    await expect(reader.tradesFrom('BTCUSDT', SCALES, { limit: 0 })).rejects.toBeInstanceOf(
      ContractViolation,
    );
  });
  ...
  it('refuses a page containing a trade below the requested cursor', async () => {
    // fromId is inclusive, so a row beneath it is outside the range that was asked for.
    // Letting it through would advance the cursor past history this page never covered.
    const { reader } = await provenVenue({
      '/api/v3/myTrades': () => json([trade(9), trade(20)]),
    });
    const failure = await reader
      .tradesFrom('BTCUSDT', SCALES, { fromId: '10', limit: 500 })
      .catch((error: unknown) => error);
    expect((failure as ContractViolation).reason).toBe('EVIDENCE_CONTRADICTORY');
  });

  it('accepts a trade exactly at the requested cursor, which fromId includes', async () => {
    // The positive control: inclusive means the boundary row belongs to this page.
    const { reader } = await provenVenue({
      '/api/v3/myTrades': () => json([trade(10), trade(11)]),
    });
    const page = await reader.tradesFrom('BTCUSDT', SCALES, { fromId: '10', limit: 500 });
    expect(page.value.trades.map((t) => t.venueTradeId)).toEqual(['10', '11']);
  });

  it('refuses a page longer than the limit it asked for', async () => {
    // The page-size bound is how a full page is told apart from the end of the range. An
    // over-long page makes that distinction meaningless.
    const { reader } = await provenVenue({
      '/api/v3/myTrades': () => json([trade(10), trade(11), trade(12)]),
    });
    const failure = await reader
      .tradesFrom('BTCUSDT', SCALES, { limit: 2 })
      .catch((error: unknown) => error);
    expect((failure as ContractViolation).reason).toBe('EVIDENCE_CONTRADICTORY');
  });

  it('accepts a page exactly at the limit, which is the full-page case', async () => {
    const { reader } = await provenVenue({
      '/api/v3/myTrades': () => json([trade(10), trade(11)]),
    });
    const page = await reader.tradesFrom('BTCUSDT', SCALES, { limit: 2 });
  ...
  it('preserves a duplicate boundary row rather than dropping it', async () => {
    ...
    const { reader } = await provenVenue({
      '/api/v3/myTrades': () => json([trade(10), trade(10)]),
    });
    const page = await reader.tradesFrom('BTCUSDT', SCALES, { limit: 500 });
    expect(page.value.trades).toHaveLength(2);
  });
